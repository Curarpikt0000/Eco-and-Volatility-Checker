...
- dashboard 底部板块(顺序):**本周KOL状态变化** → 流动性要点 → 三大央行资产负债表 → **三国货币供应量M0/M1/M2** → **三国M2十年折线** → **Credit Impulse信贷脉冲(美/中/欧,季度)** → 美国国债拍卖timeline → 外国官方托管美债(左近12月/右近10年双图) → **日本/中国分国别持有美债近10年(TIC MFH,左右双图,fetch_country_ust_holdings)** → 机构持仓13F+政要披露(5卡:佩洛西/Crenshaw/川普OGE 278-T/川普DJT Form 4/Tuberville)；每个 metric 卡片加 📌 当日短评
- ★**Credit Impulse 信贷脉冲(Chao 2026-08, 中期领先指标)**：fetch_credit_impulse() 三国(美/中/欧)。定义=新增信贷流量的【变化】÷GDP=信贷存量二阶差分/GDP,衡量【新增信贷的加速度】(非债务总量/非新增债务),领先实体经济6-9月,中国信贷脉冲=全球商品/风险资产最强领先指标之一。★口径:用BIS官方credit-to-GDP ratio(FRED QUSPAM770A/QCNPAM770A/QXMPAM770A,季度,%GDP)的【二阶差分】近似CI(GDP分母变化远慢于信贷,已验证与绝对额CRDQ*APABIS/GDP法方向拐点完全一致,数值差<0.2pp)。用ratio法优势:三国统一BIS口径国际可比+都到2025-10+避免中国季度GDP分母FRED停在2023+权威。★频率:季度更新滞后约1季,dashboard标注"季度更新·数据滞后约1季"不误导。FRED拉数走fetchers.fred.fetch_fred_history(带key API,稳)优先,回退免key fredgraph CSV(该端点偶发超时)。dashboard `_credit_impulse_svg`带零轴(正负值需基准)+最新点信号色空心圈(绿#2e9e5b/黄#e0a92e/红#d64545,SIG_META映射signal)。绝不编:某国抓不到→points=[]+status="未找到"。
- ★**KOL 半年历史导入作时序底座(Chao 2026-08)**：从另一 agent by_day DB(2025-12-18至今~205天,但每天仅6-13个KOL稀疏覆盖)一次性导出(scratch/import_kol_history.py)落盘 data/kol/daily/ 作时序底座, 与 Eco 自己每日全量91 KOL稠密快照并存(不覆盖)。用途:①kol_weekly_changes 本周vs上周真转向(伪转向过滤:direction为"未找到"/空的不算,曾误报"看空→未找到")②kol_weekly_views 加 since_date(当前方向连续保持起始日,回溯快照算)+is_new(起始日>=本周一=本周新观点)→回答"今天新说vs旧观点重复"。★现实:by_day历史稀疏(每天几个)+Eco稠密(一天全量91)粒度不同,跨源回溯易断档→当前60观点全标"本周新观点";随Eco每日攒稠密快照,首现日期会越来越准。dashboard `_kol_views_html` 每卡显示🆕本周新观点/自X日持此观点。
