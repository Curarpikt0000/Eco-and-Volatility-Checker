...
## 数据联动 (2026-08 扩展)
- **KOL 独立数据源**：从 KOL 项目 GitHub 拉名册 data/kol_registry.json(91 KOL)，Eco **每日自己 web_search 全量 91 个 KOL** 判方向→写 data/kol_independent.json(不共用另一 agent 的 Notion 结论,准确性独立)。★**2026-08 完全独立化(Chao要求)**: 每日 crawl 后 `save_kol_daily_snapshot()` 把全量 all(91条方向)落盘 data/kol/daily/YYYY-MM-DD.json(进 git, Eco 自己的数据仓库副本), `kol_weekly_changes()` 读这些快照做**本周最新 vs 上周最后**对比算转向, `kol_stance_changes_grouped()` 数据源=这些快照, **彻底不碰另一 agent 的 by_day DB**(fetch_kol_recent/kol_stance_changes 已弃用不再是主源)。快照不足跨周时诚实回退最新vs最早; 只1天时返回空(绝不编转向)。dashboard 标题=**"本周 KOL 状态变化"**。
...
- dashboard 底部板块(顺序):**本周KOL状态变化** → 流动性要点 → 三大央行资产负债表 → **三国货币供应量M0/M1/M2** → 美国国债拍卖timeline → 外国官方托管美债(左近12月/右近10年双图) → **日本/中国分国别持有美债近10年(TIC MFH,左右双图,fetch_country_ust_holdings)** → 机构持仓13F+政要披露(5卡:佩洛西/Crenshaw/川普OGE 278-T/川普DJT Form 4/Tuberville)；每个 metric 卡片加 📌 当日短评
- ★**KOL 状态变化演进 (2026-08)**：v1"当日"(太空)→v2"上周一至今"(靠另一agent by_day DB回看)→**v3"本周"完全独立(Chao最终要求, 见上方"KOL独立数据源")**: 数据源改 Eco 自己每日快照 data/kol/daily/, kol_weekly_changes 本周最新vs上周最后对比, 不再碰另一agent DB。仿13F按sector模块化(_KOL_SECTOR_MAP中英映射分组+_KOL_SECTOR_COLOR彩色标题+转向数徽章)保留;`_kol_changes_html`:顶部总览+每sector一h-module(复用13F.h-module)+每KOL卡片▲转多(clay红)/▼转空(sage绿逆向买点)徽章+prev→new方向色块+言论+标的。向后兼容旧list结构。
