...
- dashboard 底部板块(顺序):当日KOL状态变化 → 流动性要点 → 三大央行资产负债表 → **三国货币供应量M0/M1/M2** → 美国国债拍卖timeline → 外国官方托管美债 → 机构持仓13F+政要披露(5卡:佩洛西/Crenshaw/川普OGE 278-T/川普DJT Form 4/Tuberville)；每个 metric 卡片加 📌 当日短评
- ★**KOL 状态变化改"上周至今模块化"(2026-08, Chao要求)**：原"当日KOL状态变化"当日常只有1-2条太空→改成**上周一至今**(external_data._days_since_last_monday 动态算天数,约7-14天)的累积转向,**仿13F报告按sector模块化**。数据源:kol_stance_changes_grouped(days)→主源另一agent的Notion by_day DB(逐日时序,支持跨天回看,fetch_kol_recent)+补充independent.json当日changes;按sector归一(_KOL_SECTOR_MAP中英映射:Precious Metals/贵金属→贵金属,Macro→宏观货币与金融体系...)分组,每模块彩色标题(_KOL_SECTOR_COLOR)+转向数徽章。dashboard `_kol_changes_html`重写:顶部总览(X位转向Y模块)+每sector一个h-module(复用13F的.h-module样式)+每KOL卡片带▲转多(clay红)/▼转空(sage绿,逆向买点)徽章+prev→new方向色块+言论comments+标的。向后兼容旧list结构。daily cron步骤6 kc=e.kol_stance_changes_grouped()(替换旧load_independent_kol只当日2条)。当前上周至今27转向6模块。
